...
<footer id="kontak" class="bbh-public-footer scroll-mt-[84px] bg-[#10231a] px-6 pb-4 pt-7 text-white sm:px-8 lg:px-10">
    <div class="mx-auto max-w-[1184px]">
        <div class="flex items-center gap-3">
            ...
            <p class="bbh-small font-semibold">Bumiku Bumimu Hijau Farm</p>
        </div>

        <div class="mt-5 grid gap-6 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr]">
            <div>
                <p class="bbh-small font-semibold">{{ $copy['footer']['farm'] }}</p>
            ...
            <details class="group relative sm:w-[320px]">
                ...
                <div class="absolute bottom-12 right-0 z-20 w-full overflow-hidden rounded-[14px] border border-white/10 bg-[#182430] py-2 shadow-none">
                    @foreach ($languages as $locale => $label)
                        <a href="{{ \App\Support\PublicSiteCopy::switchUrl($locale) }}" class="bbh-small block px-4 py-3 text-white/75 transition hover:bg-white/10 hover:text-white {{ $currentLocale === $locale ? 'bg-white/10 text-white' : '' }}">
                            {{ $label }}
                        </a>
                    @endforeach
                </div>
